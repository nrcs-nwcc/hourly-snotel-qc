...
for year in range(2006, 2008): # this loops i through from WYstart (included) to WYend (not included) in 1 year chunks
    #read L0 data
    snotel_hourly = pd.read_csv('Level0_SNOTEL/'+stationid+'_WY'+str(year)+'_L0.csv', parse_dates=['date'])
    ...
    snotel_daily = pd.read_csv('SNOTEL_D/sd_' + stationid + '_WY' + str(year) + '.csv', parse_dates=['Date']) #data is start of day values
    ...
    df_qc['sh_SWE_mm_L1'] = np.where((df_qc['sh_SWE_mm_L1'].diff() > 25) , np.nan, df_qc['sh_SWE_mm_L1'])
    df_qc['sh_SWE_mm_L1'] = np.where((df_qc['sh_SWE_mm_L1'].diff() < -25) , np.nan, df_qc['sh_SWE_mm_L1'])
    #Set L1 QC Flag
    ...
    #### Snow Depth ####
        #Range check
    df_qc['sh_snowdepth_cm_L1'] = np.where((df_qc['sh_snowdepth_cm'] < depthmin) , np.nan, df_qc['sh_snowdepth_cm'])
    df_qc['sh_snowdepth_cm_L1'] = np.where((df_qc['sh_snowdepth_cm'] > depthmax) , np.nan, df_qc['sh_snowdepth_cm_L1'])
        #Rate of change check
    df_qc['sh_snowdepth_cm_L1'] = np.where((df_qc['sh_snowdepth_cm'].diff() > 25) , np.nan, df_qc['sh_snowdepth_cm_L1'])
    df_qc['sh_snowdepth_cm_L1'] = np.where((df_qc['sh_snowdepth_cm'].diff() < -25) , np.nan, df_qc['sh_snowdepth_cm_L1'])
        #Fill missing data caused by painting during snowfall with linear interpolation
    df_qc['sh_snowdepth_cm_L1'] = df_qc['sh_snowdepth_cm_L1'].interpolate(method ='linear', limit_direction ='backward')
       #Set L1 QC Flag
    ...
    df_qc['sh_precip_mm_L1'] = np.where((df_qc['sh_precip_mm_L1'] > precipseasonmax), np.nan, df_qc['sh_precip_mm_L1'])
    ...
    # df_qc['sh_precip_mm_L1'] = np.where((df_qc['sh_precip_mm'].diff() < -100) , np.nan, df_qc['sh_precip_mm_L1'])
        #Range check QC Flag
    df_qc['sh_precip_qc_L1'] = np.where((df_qc['sh_precip_mm_L1'] < precipseasonmin), 'S', df_qc['sh_precip_qc'])
    df_qc['sh_precip_qc_L1'] = np.where((df_qc['sh_precip_mm_L1'] > precipseasonmax), 'S', df_qc['sh_precip_qc_L1'])
    ...
    # Export
    level1.to_csv('/Users/anne/OneDrive/Data/Level1_SNOTEL/'+stationid+'_WY'+str(year)+'_L1.csv')

    ##### Level 2 #####
    #### SWE ####
    df_qc['sh_SWE_mm_L2'] = df_qc['sh_SWE_mm_L1'].interpolate(method ='linear', limit_direction ='backward', limit = 24) #linear interpolation of data gaps but never more than 24 hours
        # Calculate guides values for dynamic smoothing and manual QC process
    df_qc['sh_SWE_auto6h_mm'] = df_qc['sh_SWE_mm_L2'].rolling(6, center=True, closed='right').median() 
    df_qc['sh_SWE_6hdiff'] = df_qc['sh_SWE_auto6h_mm'].diff() #to fill in manual difference in excell once SWE is verified
    df_qc['sh_SWE_auto24h_mm'] = df_qc.sh_SWE_mm_L2.rolling(24, center=True, closed='right').median() 
    ...
    df_qc['sh_SWE_mm_L2'] = np.where((df_qc['sh_SWE_24hdiff'] <= 0), df_qc['sh_SWE_auto24h_mm'], df_qc['sh_SWE_mm_L2']) #if hourly SWE has no change or is decreasing over 24 hours, apply 24 hr rolling median to smooth diurnal flutter
    df_qc['sh_SWE_mm_L2'] = np.where((df_qc['sd_SWE_mm'].diff() <= 0), df_qc['sh_SWE_auto24h_mm'], df_qc['sh_SWE_mm_L2']) #if daily SWE has no change or is decreasing, then apply 24 hr rolling median to smooth diurnal flutter - yes, you need both versions!
    df_qc['sh_SWE_mm_L2'] = np.where((df_qc['sd_temp_max_C'] > 7), df_qc['sh_SWE_auto24h_mm'], df_qc['sh_SWE_mm_L2']) #Smooth diurnal flutter for any day with maximum air temp over 7 °C
    df_qc['sh_SWE_mm_L2'] = np.where((df_qc['sd_SWE_mm'] == 0), 0, df_qc['sh_SWE_mm_L2']) #if daily QC product is 0 then set all hourly data to 0
     #QA/QC Flag
    df_qc['sh_SWE_qc_L2'] = np.where((df_qc['sh_SWE_mm_L2'] == np.nan), 'S', df_qc['sh_SWE_qc_L1'])
    df_qc['sh_SWE_qc_L2'] = np.where((df_qc['sh_SWE_mm'] == df_qc['sh_SWE_mm_L2']) & (df_qc['sh_SWE_mm_L2']!="S"), "V", 'E') #if Level 2 data equals the origional data and the flag has not been set to suspect, then flag as "V", otherwise the data has been edited "E".
    df_qc['sh_SWE_qa_L2'] = 'F' #Flag data QA as passing automated QC

    #### Snow Depth ####
    df_qc['sh_snowdepth_cm_L2'] = df_qc['sh_snowdepth_cm_L1'].interpolate(method ='linear', limit_direction ='backward', limit = 24) #linear interpolation of data gaps but never more than 24 hours
        #Calculate median values for manual QC
    df_qc['sh_snowdepth_auto6h_cm'] = df_qc.sh_snowdepth_cm_L2.rolling(6, center=True, closed='right').median()
    df_qc['sh_snowdepth_6hdiff'] = df_qc['sh_snowdepth_auto6h_cm'].diff()
    df_qc['sh_snowdepth_auto12h_cm'] = df_qc.sh_snowdepth_cm_L2.rolling(12, center=True, closed='right').median()
    df_qc['sh_snowdepth_12hdiff'] = df_qc['sh_snowdepth_auto12h_cm'].diff()
    df_qc['sh_snowdepth_auto24h_cm'] = df_qc.sh_snowdepth_cm_L2.rolling(24, center=True, closed='right').median()
    df_qc['sh_snowdepth_24hdiff'] = df_qc['sh_snowdepth_auto24h_cm'].diff()
        #Dynamic smoothing
    df_qc['sh_snowdepth_cm_L2'] = df_qc['sh_snowdepth_auto6h_cm']
    df_qc['sh_snowdepth_cm_L2'] = np.where((df_qc['sh_snowdepth_24hdiff'] <= 0), df_qc['sh_snowdepth_auto24h_cm'], df_qc['sh_snowdepth_cm_L2']) #no change in depth or compaction/melting then apply 24 hr rolling median to smooth diurnal flutter
    df_qc['sh_snowdepth_cm_L2'] = np.where((df_qc['sd_temp_max_C'] > 7), df_qc['sh_snowdepth_auto24h_cm'], df_qc['sh_snowdepth_cm_L2']) #days with max temp above 7C has no precip according to CSSL findings so smooth diurnal flutter
    ...
    df_qc['sh_snowdepth_qc_L2'] = np.where((df_qc['sh_snowdepth_cm'] == df_qc['sh_snowdepth_cm_L2']) & (df_qc['sh_snowdepth_qc_L2']!="S"), "V", 'E') #if Level 2 data equals the origional data and the flag has not been set to suspect, then flag as "V", otherwise the data has been edited "E".
    df_qc['sh_snowdepth_qa_L2'] = 'F' #Flag data QA as passing automated QC

    #### Precip - EXPERIMENTAL ####
    df_qc['sh_precip_mm_L2'] = df_qc['sh_precip_mm_L1'].interpolate(method ='linear', limit_direction ='backward', limit = 24) #backfill remaining data gaps
     #Calculate median values for manual QC
    df_qc['sh_precip_auto24h_mm'] = df_qc.sh_precip_mm_L2.rolling(24, center=True, closed='right').median()
    df_qc['sh_precip_24hrdiff'] = df_qc['sh_precip_auto24h_mm'].diff() 
        #Dynamic smoothing
    df_qc['sh_precip_mm_L2'] = df_qc.sh_precip_mm_L2.rolling(6, center=True, closed='right').median()
    df_qc['sh_precip_mm_L2'] = np.where((df_qc['sh_precip_24hrdiff'] <= 0), df_qc['sh_precip_auto24h_mm'], df_qc['sh_precip_mm_L2']) #no change in precip or sligth decrease from temp swing then apply 24 hr rolling median to smooth diurnal flutter
    df_qc['sh_precip_mm_L2'] = np.where((df_qc['sd_temp_max_C'] > 7), df_qc['sh_precip_auto24h_mm'], df_qc['sh_precip_mm_L2']) #days with max temp above 7C has no precip according to CSSL findings so smooth diurnal flutter
    ...
    df_qc['sh_precip_mm_L2'] = np.where((df_qc['sd_precip_24hr_mm'] == 0), 0, df_qc['sh_precip_mm_L2'])
    df_qc['sh_precip_mm_L2'] = np.where((df_qc['sh_precip_mm_L2'] < -10), 0, df_qc['sh_precip_mm_L2']) #remove major decreases in precip data
    df_qc['sh_precip_mm_L2'] = np.where((df_qc['sh_precip_mm_L2'] > 50), 0, df_qc['sh_precip_mm_L2']) #remove maintenance data but will also remove major snow plug release data
    ...
    df_qc['sh_precip_mm_L2'] = df_qc['sh_precip_mm_L2'].cumsum() #add it all up
        #QA/QC Flag
    df_qc['sh_precip_qc_L2'] = np.where((df_qc['sh_precip_mm_L2'] == np.nan), 'S', df_qc['sh_precip_qc_L1'])
    df_qc['sh_precip_qc_L2'] = np.where((df_qc['sh_precip_mm'] == df_qc['sh_precip_mm_L2']) & (df_qc['sh_precip_qc_L2']!="S"), "V", 'E') #if Level 2 data equals the origional data and the flag has not been set to suspect, then flag as "V", otherwise the data has been edited "E".
    ...
    # Export
    level2.to_csv('/Users/anne/OneDrive/Data/Level2_SNOTEL/'+stationid+'_WY'+str(year)+'_L2.csv')
    ...
    fig.add_trace(go.Scatter(x=df_qc['date'],y=df_qc['sh_SWE_mm_L1'], line=dict(color='cornflowerblue'), opacity = 0.15, name='L1 SWE', yaxis='y1'))
    fig.add_trace(go.Scatter(x=df_qc['date'],y=df_qc['sh_SWE_mm_L2'], line=dict(color='deeppink'), opacity=0.5, name='L2 SWE', yaxis='y1'))
    ...
    fig.add_trace(go.Scatter(x=df_qc['date'], y=df_qc['sh_snowdepth_cm_L1'], line=dict(color='darkolivegreen'), opacity=0.15, name='L1 depth', yaxis='y1'))
    fig.add_trace(go.Scatter(x=df_qc['date'],y=df_qc['sh_snowdepth_cm_L2'], line=dict(color='deeppink'), opacity=0.5, name='L2 depth', yaxis='y1'))
    ...
    fig.add_trace(go.Scatter(x=df_qc['date'],y=df_qc['sh_precip_mm_L1'], line=dict(color='teal'), opacity=0.15, name='L1 precip', yaxis='y1'))
    fig.add_trace(go.Scatter(x=df_qc['date'],y=df_qc['sh_precip_mm_L2'], line=dict(color='deeppink'), opacity=0.5, name='L2 precip', yaxis='y1'))
    ...
    level2QC = level2QC[['date', 'sh_SWE_mm', 'sh_SWE_qc', 'sh_SWE_auto6h_mm', 'sh_SWE_6hdiff', 'sh_SWE_auto24h_mm', 'sh_SWE_24hdiff', 'sh_SWE_mm_L2','sh_SWE_L2diff', 'sh_SWE_mm_L2',  'sh_SWE_qc_L2', 'sh_SWE_qa_L2', 
                        'sh_snowdepth_cm', 'sh_snowdepth_qc', 'sh_snowdepth_auto6h_cm', 'sh_snowdepth_auto12h_cm', 'sh_snowdepth_auto24h_cm', 'sh_snowdepth_cm_L2', 'sh_snowdepth_cm_L3', 'sh_snowdepth_qc_L2', 'sh_snowdepth_qa_L2', 
                        'sh_precip_mm', 'sh_precip_diff','sh_precip_qc', 'sh_precip_auto24h_mm', 'sh_precip_24hrdiff', 'sh_precip_mm_L2', 'sh_precip_L2diff', 'sh_precip_mm_L3', 'sh_precip_qc_L2', 'sh_precip_qa_L2', 
    ...
    level2QC.columns = ['date', 'sh_SWE_mm', 'sh_SWE_qc','sh_SWE_auto6h_mm', 'sh_SWE_6hdiff', 'sh_SWE_auto24h_mm', 'sh_SWE_24hdiff', 'sh_SWE_mm_L2','sh_SWE_L2diff', 'sh_SWE_mm_L3',  'sh_SWE_qc_L3', 'sh_SWE_qa_L3', 
                        'sh_snowdepth_cm', 'sh_snowdepth_qc', 'sh_snowdepth_auto6h_cm', 'sh_snowdepth_auto12h_cm', 'sh_snowdepth_auto24h_cm', 'sh_snowdepth_cm_L2', 'sh_snowdepth_cm_L3', 'sh_snowdepth_qc_L3', 'sh_snowdepth_qa_L3', 
                        'sh_precip_mm', 'sh_precip_diff', 'sh_precip_qc', 'sh_precip_auto24h_mm', 'sh_precip_24hrdiff', 'sh_precip_mm_L2', 'sh_precip_L2diff', 'sh_precip_mm_L3', 'sh_precip_qc_L3', 'sh_precip_qa_L3', 
    ...
    # export
    level2QC.to_csv('/Users/anne/OneDrive/Data/Level2QC_SNOTEL/'+stationid+'_WY'+str(year)+'_L2QC.csv')
